...
export default function FaqSection() {
    ...
    return (
        <div className="mx-auto w-full max-w-[500px] rounded-lg p-3">
            {dataArr.map((PerAccordion, idx) => (
                <div key={idx} className="border-b border-gray-500/50 py-3 last-of-type:border-b-0">
                    <button onClick={() => toggle(idx)} className="flex h-full w-full items-center justify-between font-medium text-black outline-none dark:text-white">
                        <span>{PerAccordion.title}</span>
                        <span className="rounded-full">
                            <svg className="ml-8 size-3 shrink-0 dark:fill-white" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div
                        className={`grid overflow-hidden text-zinc-500 transition-all duration-300 ease-in-out ${isOpen === idx ? 'grid-rows-[1fr] pb-1 pt-3 opacity-100' : 'grid-rows-[0fr] opacity-0'}`}
                    >
